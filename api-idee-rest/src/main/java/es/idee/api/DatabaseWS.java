...
import es.idee.database.persistence.domain.Columna;
import es.idee.database.persistence.domain.DatosTabla;
import es.idee.database.persistence.domain.Pagina;
import es.idee.database.persistence.domain.Tabla;
import es.idee.database.service.DatabaseService;
import es.idee.database.service.impl.DatabaseServiceImpl;
...
public class DatabaseWS {
	...
	private JSONArray addLinksToReponse(String url, String formato) {
		...
		String ideeUrl = configProperties.getString("idee.url") + "api/database";
		JSONArray links = new JSONArray();
		links.put(createLink(ideeUrl + url, "self", getTypeLinkByFormato(formato), "this document"));
		addAlternateLinks(links, ideeUrl + url, formato);
